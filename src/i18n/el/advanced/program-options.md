...
Επιλογή                      | Περιγραφή
---                         | ---
`-A` ή `--appdata=`        | Διαδρομή προς τον κατάλογο home του dcrd ($HOME/.dcrd)
`-V` ή `--version`         | Εμφάνιση πληροφοριών έκδοσης και έξοδος
`-C` ή `--configfile=`     | Διαδρομή προς το αρχείο ρυθμίσεων
`-b` ή `--datadir=`        | Κατάλογος για την αποθήκευση δεδομένων
`--logdir=`                 | Κατάλογος για το αρχείο καταγραφής εξόδου. ($HOME/.dcrd/logs)
`-a` ή `--addpeer=`        | Προσθέστε ένα peer για να συνδεθείτε μαζί του κατά την εκκίνηση
`--connect=`                | Συνδεθείτε μόνο με τους συγκεκριμένους peers κατά την εκκίνηση
`--nolisten`                | Απενεργοποίηση της ακρόασης για τις εισερχόμενες συνδέσεις - ΣΗΜΕΙΩΣΗ: Η ακρόαση απενεργοποιείται αυτόματα αν χρησιμοποιούνται οι επιλογές `--connect` ή `--proxy` χωρίς να διευκρινίζονται επίσης οι διεπαφές ακρόασης μέσω του `--listen`
`--listen=`                 | Προσθέστε μια διασύνδεση/θύρα για να ακούσετε για συνδέσεις (προεπιλογή όλων των θυρών διασύνδεσης: `9108`, testnet: `19108`)
`--maxpeers=`               | Μέγιστος αριθμός εισερχόμενων και εξερχόμενων peers (125)
