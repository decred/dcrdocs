...
기본 옵션은 대부분의 사용자에게는 정상입니다.
자격 증명을 설정하십시오. 이것은 `dcrd` 가
대부분의 사용자에게 '기본 제공'기능을 제공합니다. 그러나, 또한 넓은 있습니다
행동을 제어하는 ​​데 사용할 수있는 다양한 플래그. 다음 절
플래그를 열거하는 사용 개요를 제공합니다. 흥미로운
주목할 점은 이러한 모든 옵션의 긴 형태 (
`-C`) 는 자동으로 설정 파일에 지정 될 수있습니다.
dcrd가 시작될 때 구문 분석됩니다.
 `-C` (`--configfile`)
플래그는 아래와 같이 기본 위치를 덮어 쓸 수 있습니다. `-h` 플래그를 사용하여 항상 모든 옵션 목록을 얻을 수 있습니다.
...
옵션                      | 기술
---                         | ---
`-A` 또는 `--appdata=`        | dcrd 홈 디렉토리 경로 ($ HOME / .dcrd)
`-V` 또는 `--version`         | 버전 정보 표시 및 종료
`-C` 또는 `--configfile=`     | 구성 파일의 경로
`-b` 또는 `--datadir=`        | 데이터를 저장할 디렉토리
`--logdir=`                 | 로그 출력을위한 디렉토리. ($ HOME / .dcrd / logs)
`-a` 또는 `--addpeer=`        | 시작시 연결할 친구 추가
`--connect=`                | 시작할 때 지정된 친구에게만 연결
`--nolisten`                | 들어오는 연결을 수신하지 않음 -주의 : `--connect` 또는 `--proxy` 옵션이 `--listen` 을 통해 청취 인터페이스를 지정하지 않고 사용되면 청취가 자동으로 비활성화됩니다
`--listen=`                 | 연결을 기다리는 인터페이스 / 포트를 추가하십시오 (기본값은 모든 인터페이스 포트 : `9108`, testnet: `19108`)
`--maxpeers=`               | 인바운드 및 아웃 바운드 친구 최대 수 (125)
