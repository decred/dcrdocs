...
In the BIP32 path, Coin type is a constant, set for each cryptocoin. [BIP-0032](https://github.com/bitcoin/bips/blob/master/bip-0032.mediawiki) is a Bitcoin Improvement Proposal, which introduces [Hierarchical Deterministic (HD) Wallets](#hd-wallet). Coin Type was introduced in [BIP-0044](https://github.com/bitcoin/bips/blob/master/bip-0044.mediawiki), but the registered coins are stored in a Satoshi Labs Improvement Proposal, in [SLIP-0044](https://github.com/satoshilabs/slips/blob/master/slip-0044.md). Every coin has an index. The index of Decred is 42.
...
Wallets using the HD protocol are called HD wallets. The Hierarchical Deterministic (HD) key creation and transfer protocol ([BIP-0032](https://github.com/bitcoin/bips/blob/master/bip-0032.mediawiki)) allows creating child keys from parent keys in a hierarchy. There are two posible types of BIP32 derivation, hardened and non-hardened. With non-hardened keys, you can prove a child public key is linked to a parent public key using just the public keys. For security reasons, using hardened keys is safer, but there are use cases for using non-hardened keys.
In the BIP32 path, hardened derivation is indicated by apostrophe (').
