...
Oletusasetukset ovat järkevät suurimmalle osalle käyttäjistä, tosin saatat joutua 
asettamaan käyttäjätietosi. Tämä tarkoittaa että `dcrd` toimii
'sellaisenaan' useimmille käyttäjille. On olemassa kuitenkin suuri
määrä flägejä joita voidaan käyttää ohjelman käyttäytymisen kontrollointiin. Seuraavasta osiosta
löydät perusohjeita käyttöön ja luettelon flägeistä. Kiintoisa 
asia huomioida on se, että kaikkien näiden komentojen pitkä muoto (paitsi
`-C`) voidaan spesifioida konfiguraatiotiedostossa, jolloin se käsitellään
automaattisesti kun dcrd käynnistyy.
 `-C` (`--configfile`)
flägiä, kuten nähtävissä alla, voidaan käyttää oletus-sijainnin ohittamiseen.
Voit koska tahansa nähdä listan kaikista komennoista käyttäen `-h` flägiä.
...
Komento                      | Kuvaus
---                         | ---
`-A` tai `--appdata=`        | Polku dcrd kansioon ($HOME/.dcrd)
`-V` tai `--version`         | Näytä version tiedot ja lopeta
`-C` tai `--configfile=`     | Konfiguraatiotiedoston polku
`-b` tai `--datadir=`        | Datan säilytyskansio
`--logdir=`                 | Kansio johon kerätä loki outputeista ($HOME/.dcrd/logs)
`-a` tai `--addpeer=`        | Lisää käyttäjä johon ottaa yhteys käynnistyksen yhteydessä
`--connect=`                | Ota yhteys vain määriteltyihin käyttäjiin käynnistyksen yhteydessä
`--nolisten`                | Ota sisääntulevien yhteyksien kuuntelu pois käytöstä -- HUOM: Kuuntelu on automaattisesti pois käytöstä jos `--connect` tai `--proxy` komentoja on käytetty spesifioimatta kuunteluinterfaceja käyttämällä `--listen` komentoa
`--listen=`                 | Lisää uusi interface/portti kuuntelemaan yhteyksiä (oletus kaikille interfaceille: portti `9108`, testnet: `19108`)
`--maxpeers=`               | Maksimimäärä yhteyksiä sisään ja ulos (125) 
