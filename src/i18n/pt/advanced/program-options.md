...
A operação padrão é razoável para maioria dos usuários, embora você talvez precise
informar suas credenciais. Isto significa que `dcrd` vai
funcionar sem problemas para maioria dos usuários. Entretanto, existe também uma grande
quantidade de flags que podem ser usadas para controlar seu comportamento. A seção seguinte
disponibiliza uma lista dessas flags. É interessante
notar que a forma longa destas opções , (com excessão de
`-C`) pode ser definida em um arquivo de configuração que é automaticamente
parseado quando dcrd é iniciado.
A flag `-C` (`--configfile`),
como mostrado abaixo, pode ser usado para sobrescrever a localização padrão.
Você pode visualizar a lista com todas as opções usando a flag `-h`. 
...
Opção                      | Descrição
---                         | ---
 `-A` ou `--appdata=`        | Caminho para dcrd home directory ($HOME/.dcrd)
`-V` ou `--version`         | Exibir informações da versão e sair
`-C` ou `--configfile=`     | Caminho para o arquivo de configuração
`-b` ou `--datadir=`        | Diretório para armazenar dados
`--logdir=`                 | Pasta do log de output. ($HOME/.dcrd/logs)
`-a` ou `--addpeer=`        | Adicionar um peer com o qual se conectar na inicialização
`--connect=`                | Conectar-se apenas aos peers especificados na inicialização
`--nolisten`                | Desativar a escuta de conexões recebidas - NOTA: A escuta é desabilitada automaticamente se as opções `--connect` ou` --proxy` forem usadas sem especificar interfaces de escuta através de `--listen`
`--listen=`                 | Adicionar uma interface/porta para escutar conexões (padrão de todas as interfaces porta:`9108`, testnet: `19108`)
`--maxpeers=`               | Número máximo de peers de entrada e saída (125)
