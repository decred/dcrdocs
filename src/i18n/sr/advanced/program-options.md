...
Подразумеване опције су исте за већину корисника, иако ће можда бити потребно
поставити акредитиве. То значи да ће `dcrd` радити у свакој ситуацији за већину корисника. Међутим, постоје разне ознаке којима се може контролисати његова употреба. Следећи одељак пружа преглед ових ознака. Занимљиво је напоменути да дуги облик свих ових опција (осим
`-C`) могу бити специфицирани у конфигурационој датотеки која је аутоматски
раздвојена кад се дцрд покрене.
`-C` (`--configfile`)
ознака, као што је приказано испод, може се користити за превазилажење подразумеване локације.
Увек можете добити листу свих опција користећи `-h` ознаку.
...
Опис                      | опција
---                         | ---
`-A` или `--appdata=`        | Пут до дцрд почетног директоријума ($ ХОМЕ / .дцрд)
`-V` или `--version`         | Приказ информација о верзији и излаз
`-C` или `--configfile=`     | Путања до конфигурационог фајла
`-b` или `--datadir=`        | Именик за чување података
`--logdir=`                 | Именик директоријума за логовање. ($ ХОМЕ / .дцрд / логс)
`-a` или `--addpeer=`        | Додајте пеер за повезивање при покретању
`--connect=`                | Повежите се само са сличнима приликом покретања
`--nolisten`                | Онемогући слушање долазних веза - НАПОМЕНА: Слушање се аутоматски онемогућава ако се користе опције `--цоннецт` или` --прокси`, без потребе за спецификацијом интерфејса за слушање путем `--листен`
`--listen=`                 | Додајте интерфејс / порт да бисте слушали везе (подразумевано сви портови интерфејса: `9108`, тестнет:` 19108`)
`--maxpeers=`               | Максималан број улазних и излазних вршњака (125)
