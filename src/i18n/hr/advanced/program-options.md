...
Zadane opcije su dobre za većinu korisnika, iako možda trebate postavite vjerodajnice. To znači da će `dcrd` raditi "izvan okvira" za većinu korisnika. Međutim, tu su i raznolike zastave koje se mogu koristiti za kontrolu ponašanja. Sljedeći odjeljak daje pregled korištenja koji nabraja zastave. Zanimljiva stavka je da je dug oblik svih tih opcija (osim
`-C`) može se odrediti u konfiguracijskoj datoteci koja automatski analizira kada treba započeti dcrd.
 `-C` (`--configfile`)
zastavica, kako je dolje prikazano, može se koristiti za nadjačavanje zadane lokacije.
Uvijek možete dobiti popis svih opcija pomoću `-h` zastavice.
...
Opcija                      | Opis
---                         | ---
`-A` ili `--appdata=`        | Put do dcrd domaćeg direktorija ($ KUČIŠTE/.dcrd)
`-V` ili `--version`         | Prikaz informacija o verziji i izlaz
`-C` ili `--configfile=`     | Put do konfiguracijske datoteke
`-b` ili `--datadir=`        | Direktorij za pohranu podataka
`--logdir=`                 | Direktorij za zabilježene prijave. ($ KUČIŠTE/ .dcrd/trupci)
`-a` ili `--addpeer=`        | Dodajte vršnjake za povezivanje pri pokretanju
`--connect=`                | Pripremite se samo s navedenim vršnjacima pri pokretanju
`--nolisten`                | Onemogući slušanje dolaznih povezivanja -- NAPOMENA: Slušanje je automatski onemogućeno ako se koriste opcije `--connect` ili `--proxy` bez navođenja slušnih sučelja putem `--listen`
`--listen=`                 | Dodajte sučelje/ulaz za slušanje za povezivanja (zadano sve ulaze u sučelja: `9108`, testnet: `19108`)
`--maxpeers=`               | Maksimalni broj dolaznih i odlaznih vršnjaka (125)
