...
4. If a proposer owner decides to abandon their proposal (for instance, if comments from the community indicate it is not likely to pass), they can ask the admins to mark their proposal as ‘Abandoned’. Abandoned proposals appear in the ‘Abandoned’ tab on Politeia. They are publicly viewable, but cannot be edited, commented on, or be authorized to vote once abandoned. Admins can mark a proposal as abandoned at any time (for instance, if it is deemed spam) before the proposal owner authorizes voting. Once the proposal owner authorizes voting, it cannot be abandoned.  
...
9. Payment claims will be handled manually by Decred Holdings Group (DHG) until such times as this process can be handled by the [Decred Decentralized Autonomous Entity (DAE)](#decentralized-autonomous-entity-dae).
