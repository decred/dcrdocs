...
A constant, set for each cryptocoin for the BIP32 path. BIP-0032 introduces [HD Wallets](#hd-wallet). In the BIP32 path, every coin has an index. The index of Decred is 42. Registered coins are stored in [SLIP-0044](https://github.com/satoshilabs/slips/blob/master/slip-0044.md). 

Before SLIP44, the list of cryptocurrencies did not exist, the only two types were Bitcoin and Bitcoin Testnet. A wallet which contains addresses that were generated by the legacy BIP44 method would contain addresses that the wallet software couldn’t see. 
