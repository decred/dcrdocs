...
Decrediton is released with the Binary Releases and can be found here: [https://github.com/decred/decred-binaries/releases/tag/v1.0.6](https://github.com/decred/decred-binaries/releases/tag/v1.0.6). As of v1.0.6, Decrediton is only available for Linux and Mac.

> macOS/OSX

1. Download the `decrediton-1.0.6.dmg` file.

2. Double click the `decrediton-1.0.6.dmg` file once downloaded to mount the disk image.

3. Drag the decrediton.app into the link to your Applications folder within the disk image.

> Linux

1. Download the `decrediton-1.0.6.tar.gz` file.
...
2. Navigate to download location and extract the .tar.gz file:
    ...
    Both of these should extract the tar.gz into a folder that shares the same name. (`e.g. tar -xvzf decrediton-v1.0.6.tar.gz` should extract to `decrediton-v1.0.6`). If successful, this new folder should include a `decrediton` executable.
