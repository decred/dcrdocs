...
or use a PoS [stake pool](https://decreg.org). (Click 'Find a stake pool' at the top right.)
...
rights to the pool. The pool does not touch your funds. This is very important since
Paymetheus is unable to vote.
...
to generate a new one for each transaction. There's around 1.4E48 (that's 1.4 followed by 48 zeroes)
