...
Параметры по умолчанию прекрасно служат большинству пользователей, хотя может потребоваться ввести свои учетные данные. Это означает, что `dcrd` будет
работать для большинства пользователей "в готовом виде". Однако существует также большое количество флагов, которые можно использовать для управления ее поведением. Следующий раздел
предоставляет обзор по использованию, в котором перечислены эти флаги. Интересно
отметить, что длинная форма всех этих параметров (кроме
`-C`) может быть указана в конфигурационном файле, который автоматически
анализируется при запуске dcrd.
Флаг `-C` (`--configfile`),
как показано ниже, может использоваться для переопределения местоположения по умолчанию. Вы всегда можете получить список всех параметров, используя флаг `-h`.
...
Параметр                      | Описание
---                         | ---
`-A` или `--appdata=`        | Путь к домашнему каталогу dcrd ($HOME/.dcrd)
`-V` или `--version`         | Отобразить информацию о версии и выйти
`-C` или `--configfile=`     | Путь к конфигурационному файлу
`-b` или `--datadir=`        | Каталог для хранения данных
`--logdir=`                 | Каталог для вывода записей журнала. ($HOME/.dcrd/logs)
`-a` или `--addpeer=`        | Добавить одноранговый узел (peer) для подключения при запуске
`--connect=`                | Подключиться только к указанным peers при запуске
`--nolisten`                | Отключить прослушивание для входящих подключений -- ПРИМЕЧАНИЯ: прослушивание автоматически отключается, если опции `--connect` или `--proxy` используются без указания также интерфейсов прослушивания через `--listen`
`--listen=`                 | Добавить интерфейс / порт для прослушивания подключений (по умолчанию порт всех интерфейсов: `9108`, testnet: `19108`)
`--maxpeers=`               | Максимальное количество входящих и исходящих peers (125)
